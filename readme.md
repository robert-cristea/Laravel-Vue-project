...
Same as any other Laravel project. Configure by editing .env file.

### Rollout migrations

As in any Laravel project. From project root execute:
...
Laraspace provide instructions on this. When developing, `npm run dev` or `npm run hot` is recommended. To prepare
a production deploy `run npm run deploy` and assets will be generated in `public` folder.
